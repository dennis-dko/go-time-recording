...
func TestAdministeredNoticesEachThingOnItsOwn(t *testing.T) {
	...
	administered := map[string]model.Telemetry{
		...
		"tracing switched off": {TraceExporter: ptr(model.TracingOff)},
		"an exporter":          {TraceExporter: ptr(model.TraceExporterOTLP)},
		"a collector only":     {TracerURL: ptr("jaeger:4317")},
		// Zero is a real choice, so it has to be noticed like any other.
		"a ratio of zero": {TracerRatio: ptr(0.0)},
	}
...
func TestAnEmptyExporterIsOffRatherThanAbsent(t *testing.T) {
	off := model.Telemetry{TraceExporter: ptr(model.TracingOff)}
...
func TestSwitchingTracingOffClearsWhatItMakesMeaningless(t *testing.T) {
	got := model.Telemetry{
		TraceExporter: ptr(model.TracingOff),
...
func TestValidationRejectsWhatWouldSilentlyDropEverySpan(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			model.Telemetry{TraceExporter: ptr("otel"), TracerURL: ptr("collector:4317")},
			"traceExporter",
		},
		{
			"zipkin is on its way out upstream and is not offered",
			model.Telemetry{TraceExporter: ptr("zipkin"), TracerURL: ptr("zipkin:9411")},
			"traceExporter",
		},
		{
			"the hosted gofr exporter would post every span to a third party",
			model.Telemetry{TraceExporter: ptr("gofr")},
			"traceExporter",
		},
		{
			"an exporter with no collector leaves tracing off while the screen shows it on",
			model.Telemetry{TraceExporter: ptr(model.TraceExporterOTLP)},
			"tracerUrl",
		},
		{
			...
			model.Telemetry{
				...
				TracerURL:     ptr("http://jaeger:4317"),
			},
			"tracerUrl",
		},
		{
			...
			model.Telemetry{
				TraceExporter: ptr(model.TraceExporterOTLP),
				TracerURL:     ptr("jaeger:4317/v1/traces"),
			},
			"tracerUrl",
		},
		{
			...
			model.Telemetry{
				TraceExporter: ptr(model.TraceExporterJaeger),
				TracerURL:     ptr("jaeger"),
			},
			"tracerUrl",
		},
		{
		...
		},
		{
			...
			model.Telemetry{
				TraceExporter: ptr(model.TraceExporterJaeger),
				TracerURL:     ptr("jaeger:70000"),
			},
			"tracerUrl",
		},
		{
			"a ratio above one would be clamped silently",
			model.Telemetry{TracerRatio: ptr(1.5)},
			"tracerRatio",
		},
		{
			"a negative ratio would be clamped to sampling nothing",
			model.Telemetry{TracerRatio: ptr(-0.1)},
			"tracerRatio",
		},
		{
			...
			model.Telemetry{TracerRatio: ptr(math.NaN())},
			"tracerRatio",
		},
		{
...
func TestValidationAcceptsWhatGoFrCanActuallyUse(t *testing.T) {
	cases := map[string]model.Telemetry{
		...
		"a collector by address": {
			TraceExporter: ptr(model.TraceExporterJaeger),
			TracerURL:     ptr("10.0.0.4:4317"),
		},
		"a collector on IPv6": {
			TraceExporter: ptr(model.TraceExporterOTLP),
			TracerURL:     ptr("[::1]:4317"),
		},
